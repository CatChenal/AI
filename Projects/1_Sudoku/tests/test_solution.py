...
class TestComputedUnits(unittest.TestCase):
    diags = ['A1','A9','B2','B8','C3','C7','D4','D6','F4','F6','G3','G7','H2','H8','I1','I9']
    center = 'E5'
    diags_boxes = diags + [center]
    
    non_diag_boxes = ['A2','A3','A4','A5','A6','A7','A8','B1','B3','B4','B5','B6','B7','B9',
                      'C1','C2','C4','C5','C6','C8','C9','D1','D2','D3','D5','D7','D8','D9',
                      'E1','E2','E3','E4','E6','E7','E8','E9','F1','F2','F3','F5','F7','F8',
                      'F9','G1','G2','G4','G5','G6','G8','G9','H1','H3','H4','H5','H6','H7',
                      'H9','I2','I3','I4','I5','I6','I7','I8']

    C2_u = [['C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8', 'C9'],
            ['A2', 'B2', 'C2', 'D2', 'E2', 'F2', 'G2', 'H2', 'I2'],
            ['A1', 'A2', 'A3', 'B1', 'B2', 'B3', 'C1', 'C2', 'C3']]
    C2_peers = set(['A2', 'B2', 'D2', 'E2', 'F2', 'G2', 'H2', 'I2',
                    'C1', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8', 'C9',
                    'A1', 'A3', 'B1', 'B3'])

    sol_boxes = solution.boxes
    sol_units = solution.extract_units(solution.unitlist, sol_boxes)
    sol_peers = solution.extract_peers(sol_units, sol_boxes)
    
    def test_check(self):
        self.assertEqual(len(self.sol_boxes), 81)
        self.assertEqual(len(solution.unitlist), 29)
        self.assertEqual(len(solution.diag_units), 2)
        
        self.assertEqual(len(self.sol_units[self.center]), 5)
        self.assertEqual(len(self.sol_peers[self.center]), 32)
        
        self.assertTrue( all(len(self.sol_units[b]) == 3 for b in self.sol_boxes if b in self.non_diag_boxes) )
        self.assertTrue( all(len(self.sol_units[b]) == 4 for b in self.sol_boxes if b in self.diags) )
        self.assertTrue( all(len(self.sol_peers[b]) == 20 for b in self.sol_boxes if b in self.non_diag_boxes) )
        self.assertTrue( all(len(self.sol_peers[b]) == 26 for b in self.sol_boxes if b in self.diags) )
        
        self.assertListEqual(self.sol_units['C2'], self.C2_u)
        self.assertSetEqual(self.sol_peers['C2'], self.C2_peers)
